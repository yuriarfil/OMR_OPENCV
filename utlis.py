...
def splitBoxesFree(img,choices,questions):
...
#def showAnswers(img,myIndex,grading,ans,questions=21,choices=5,rlQuestions=20):
def showAnswers(img,myIndex,grading,ans,rlQuestions,questions=21,choices=5):
     secW = int(img.shape[1]/choices)
     secH = int(img.shape[0]/questions)

     for x in range(0,rlQuestions):
         myAns= myIndex[x]
         cX = (myAns * secW) + secW // 2
         cY = (x * secH) + secH + secH // 2
         if grading[x]==1:
            myColor = (0,255,0)
            ...
            myColor = (0, 255, 0)
            correctAns = ans[x]
            cv2.circle(img,((correctAns * secW)+secW//2, (x * secH)+secH+secH//2), 20,myColor,cv2.FILLED)